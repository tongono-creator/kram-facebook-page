...
def generate_caption(detail, shopee, lazada, promo, highlights):
    ...
    prompt = (
        ...
        f"ห้ามขึ้นต้นด้วยวลีสูตร เช่น 'สวัสดีครับทุกคน', 'วันนี้มีสินค้า', 'ในยุคที่...', 'ปัจจุบัน...' ให้เปิดด้วยความตึงเครียด ความแปลกใจ หรือประโยชน์รูปธรรมตรงๆ ใส่ความเห็นส่วนตัวอย่างน้อย 1 ประโยค (เช่น 'ส่วนตัวรู้สึกว่า...' หรือ 'จุดที่ถูกใจคือ...') ห้ามทุกประโยคมีโครงสร้างหรือจังหวะสมมาตร\n"
    ...
    # local fallback caption (friend-style)
    ...
    first_few = " ".join([l.strip() for l in detail.splitlines() if l.strip()][:2])
    caption = (
        f"{_opener}\n\n"
        f"{first_few}\n\n"
        f"{highlights}\n\n"
